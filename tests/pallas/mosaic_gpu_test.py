...
class PallasCallTest(PallasTest, jtu.CudaArchSpecificTest):
  ...
  def test_copy_gmem_to_smem_aval_fails_mismatched_shapes(self):
    dtype = jnp.bfloat16
    @self.kernel(
    ...
    with self.assertRaisesRegex(
        ValueError,
        "Mismatch.*src.*shape.*\\(128,\\).*dst.*shape.*\\(256,\\).*",
    ):
      jax.jit(kernel).lower()

  def test_copy_smem_to_gmem_aval_fails_mismatched_shapes(self):
    dtype = jnp.bfloat16
    @self.kernel(
        out_type=jax.ShapeDtypeStruct([128], dtype),
        scratch_types=[
    ...
    with self.assertRaisesRegex(
        ValueError,
        "Mismatch.*src.*shape.*\\(256,\\).*dst.*shape.*\\(128,\\).*",
    ):
      jax.jit(kernel).lower()

  def test_copy_gmem_to_smem_aval_fails_mismatched_dtypes(self):
    @self.kernel(
    ...
    with self.assertRaisesRegex(
        ValueError,
        "Mismatch.*src.*dtype.*bfloat16.*dst.*dtype.*float32",
    ):
      jax.jit(kernel).lower()

  def test_copy_smem_to_gmem_aval_fails_mismatched_dtypes(self):
    @self.kernel(
        out_type=jax.ShapeDtypeStruct([256], jnp.bfloat16),
        scratch_types=[
            plgpu.SMEM((256,), jnp.float32),
    ...
    with self.assertRaisesRegex(
        ValueError,
        "Mismatch.*src.*dtype.*float32.*dst.*dtype.*bfloat16",
    ):
      jax.jit(kernel).lower()
